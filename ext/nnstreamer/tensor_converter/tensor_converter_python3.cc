 ...
 * @author	Gicchan Jang <gichan2.jang@samsung.com>
...
class PYConverterCore
{
  ...
  int parseTensorsInfo (PyObject *result, PyObject *tensor_type, GstTensorsInfo *info);
 ...
 * @param[config] config Structure for output tensors config
 * @return 0 if no error, otherwise negative errno
 */
int
PYConverterCore::parseTensorsInfo (PyObject *result, PyObject *tensor_type, GstTensorsInfo *info)
{
  ...
  for (unsigned int i = 0; i < info->num_tensors; i++) {
    ...
    info->info[i].type = (_nns_tensor_type) PyLong_AsLong (PyList_GetItem (tensor_type, (Py_ssize_t)i));
...
PYConverterCore::convert (GstBuffer *in_buf, GstTensorsConfig *config)
{
  ...
  PyObject *tensors_info = NULL, *output = NULL, *tensor_type = NULL, *pyValue = NULL;
  gint rate_n, rate_d;
  guint mem_size;
  gpointer mem_data;
  if (nullptr == in_buf)
  ...
  if (!gst_memory_map (in_mem, &in_info, GST_MAP_READ)) {
    ml_loge ("Cannot map input memory / tensor_converter::custom-script");
    return NULL;
  }
  ...
  if (!PyObject_HasAttrString (core_obj, (char *)"convert")) {
    Py_ERRMSG ("Cannot find 'invoke'");
    goto done;
  }

  pyValue = PyObject_CallMethod (core_obj, "convert", "(O)", param);

  if (!PyArg_ParseTuple (pyValue, "OOOii", &tensors_info, &output,
      &tensor_type, &rate_n, &rate_d)) {
    ml_loge ("Failed to parse converting result");
    goto done;
  }
  Py_UNLOCK ();

  if (parseTensorsInfo (tensors_info, tensor_type, &config->info) != 0) {
    ml_loge ("Failed to parse tensors info");
    goto done;
  }
 ...
 */
static int
py_open (gchar *path, void **priv_data)
{
  ...
  *priv_data = NULL;
  ...
  if (core == NULL) {
    ml_loge ("Failed to allocate memory for converter subplugin: Python\n");
    return -1;
  }

  if (core->init () != 0) {
    delete core;
    ml_loge ("failed to initailize the object: Python\n");
    return -2;
  }
  *priv_data = core;
